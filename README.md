# TP EQUIPE - Travail en equipe git/github

## Equipe
- Émile
- Nadjib
- Neil
- Bruno

## Aide-mémoire
### Règles d'Or pour l'Equipe
- Toujours pull avant de push
- Messages de commit clairs : "feat: ajout fonction afficher_menu"
- Branches descriptives : feature-menu, fix-bug-affichage
- Communication : Parler avant de merger
- Code reviews : Toujours 1 autre membre approuve

## Fonctionnalités

### Exercice 1: Premier Projet Collaboratif Simple
- [ ] Fonctionnalité 1 (par Émile):
  - [ ] Chef de projet / Git Master
  - [ ] Consodilation du codes et des méhodes
- [ ] Fonctionnalité 2 (par Nadjib):
  - [ ] Crée fonction `afficher_entres()`
  - [ ] Ajoute 3 entrées au menu
  - [ ] Pousse ses changements
- [ ] Fonctionnalité 3 (par Neil)
  - [ ] Crée une fonction `afficher_plats_principaux()`
  - [ ] Ajoute 3 plats principaux
  - [ ] Pousse ses changements
- [ ] Fonctionnalité 4 (par Bruno)
  - [ ] Crée une fonction `afficher_desserts()`
  - [ ] Ajoute 3 desserts
  - [ ] Crée le main() pour tout afficher
     
### Exercice 2 : Gestion de Bibliothèque Partagée
- [ ] Implémente `ajouter_livre()` (par Émile)
- [ ] Implémente `afficher_livres()` [Membre])
- [ ] Implémente `rechercher_livre()` (par Bruno)
- [ ] Crée les tests dans `test_bibliotheque.py` (par [Membre])

### Exercice 3 : Jeu du Pendu Collaboratif
- [ ] Implémente `mots.py` (par [Membre])
- [ ] Implémente `affichage.py` (par [Membre])
- [ ] Implémente `jeu.py` (par [Membre])
- [ ] Implémente `main.py` (par [Membre])

### Exercice 4 : Calculateur d'IMC avec Interface
- [ ] Implémente `calcul.py` (par [Membre])
- [ ] Implémente `interface.py` (par [Membre])
- [ ] Implémente `historique.py` (par [Membre])
- [ ] Implémente `main.py` (par [Membre])

### Exercice 5 : Système de Tickets de Support
- [ ] Implémente la structure [Everyone]
- [ ] Tous modifient `main.py` en même temps [Everyone]
- [ ] Résolution des conflits en équipe [Everyone]
- [ ] Revue de code croisée [Everyone]
 
### Exercice Final : Projet Libre
```
# [Nom du Projet]

## Equipe

- [Membre 1] - Rôle/Fonctionnalité
- [Membre 2] - Rôle/Fonctionnalité
- [Membre 3] - Rôle/Fonctionnalité
- [Membre 4] - Rôle/Fonctionnalité

## Installation

1. git clone [url]
2. cd [projet]
3. python main.py

## Fonctionnalités

- [ ] Fonctionnalité 1 (par [Membre])
- [ ] Fonctionnalité 2 (par [Membre])
- [ ] Fonctionnalité 3 (par [Membre])
- [ ] Fonctionnalité 4 (par [Membre])

## Leçons Apprises

- Comment résoudre les conflits Git
- L'importance de la communication
- etc.
```

## Leçons Apprises
- Comment résoudre les conflits Git
- L'importance de la communication
- etc.
